...
data Hand
  = High [Card] | OnePair [Card] | TwoPairs [Card] | ThreeOfKind [Card]
  | FullHouse [Card] | FourOfKind [Card] | FiveOfKind [Card]
  deriving (Show, Eq, Ord)
...
calculateWinning =
  sum . map (\(r,(_,b)) -> r * b) . zip [1..] . sort

task1 filename =
  calculateWinning . parseData <$> readFile filename
...
task2 filename =
  calculateWinning . parseData . replaceJ <$> readFile filename
